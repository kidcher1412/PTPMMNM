...
class Game:
	...
	def pause_menu(self, display_surface):
		menu = pygame_menu.Menu('Pause Menu', WINDOW_WIDTH*0.7, WINDOW_HEIGHT*0.7,
								theme=pygame_menu.themes.THEME_DARK)

		menu.add.button('Resume', lambda:self.unpause_game(menu))
		menu.add.button('Quit', pygame_menu.events.EXIT)

		menu.mainloop(display_surface)
		return menu.is_enabled()
	def unpause_game(self,menu):
		menu.disable()
	...
	def setup(self):
		...
		tmx_map = load_pygame('p1_setup/map/map1.tmx')
		...
		while True:
			...
			for event in pygame.event.get():
					if event.type == pygame.QUIT:
						...
						if event.key == pygame.K_u:  # Nhấn u để hiển thị menu pause
							self.pause_menu(self.display_surface)
