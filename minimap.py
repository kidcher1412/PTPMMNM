...
class Map(pygame.sprite.Sprite):
    def __init__(self, screen):
        ...
        self.original_image = pygame.image.load('graphics/artmaps/map1.png').convert_alpha()
    ...
    def draw_map_preview(self, map_data,map_preview_surface):
        ...
        image_position = ((map_preview_surface.get_width() - zoomed_size[0]) // 2, (map_preview_surface.get_height() - zoomed_size[1]) // 2)

        self.background_rect.topleft = ((map_preview_surface.get_width() - self.map_preview['size'][0]) // 2, (map_preview_surface.get_height() - self.map_preview['size'][1]) // 2)

        # Vẽ phông dưới ảnh
        pygame.draw.rect(map_preview_surface, self.background_color, self.background_rect)

        # Giới hạn phạm vi vẽ của ảnh bằng background_rect
        map_preview_surface.set_clip(self.background_rect)

        # Vẽ ảnh đã zoom
        map_preview_surface.blit(zoomed_image, image_position)

        # Tính toán vị trí mới của chấm
        dot_position_on_surface = (self.dot_position[0] * self.zoom_factor + image_position[0], self.dot_position[1] * self.zoom_factor + image_position[1])

        # Vẽ chấm tại tọa độ dot_position
        pygame.draw.circle(map_preview_surface, (255, 0, 0), (int(dot_position_on_surface[0]), int(dot_position_on_surface[1])), 5)

        # Trả lại phạm vi vẽ ban đầu
        map_preview_surface.set_clip(None)
        # Vẽ khung
        pygame.draw.rect(map_preview_surface, (150, 150, 150), self.background_rect, 5)
